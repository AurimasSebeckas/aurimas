...
for(;currentGen<=totalGen; currentGen++){
    if (currentGen % 2 ==0){  
        if (currentGen < 5){  
            totalMW=62+totalMW
            console.log( `Generatorius ${currentGen} įjungtas, pridėjo 62Mw,  viso generuojama  ${totalMW}  Mw!`)
        }  else {
            totalMW=124+totalMW
            console.log( `Generatorius ${currentGen} įjungtas, pridėjo 124Mw,  viso generuojama  ${totalMW}  Mw!`)
        }    
    }   else {
...
let c = (26-32)*0.5556
console.log(tekstas.substr(0,20)+c.toFixed(1)+tekstas.substr(19)  )*/
/*
Užduotis 10
...
function ilgiausias (fraze){
    let frazesmasyvas = fraze.split(" ")
    let ilgiausiazodis = " "
    for ( let i = 0; i<frazesmasyvas.length; i++){
        if (frazesmasyvas[i].length>ilgiausiazodis.length){
            ilgiausiazodis = frazesmasyvas[i] 
        }
    }
    return ilgiausiazodis
}
let galutinefraze = ""
for ( let i = 0; i<posts.length; i++){
    galutinefraze = galutinefraze + ilgiausias(posts[i]) + " "
}
console.log(galutinefraze)
/*
Užduotis 11
